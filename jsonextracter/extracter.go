...
func BuildExtracter(path string, allowMissingKeys bool) (Extracter, error) {
	parser, err := Parse(path, path)
	if err != nil {
		return nil, fmt.Errorf("error in parsing path %q: %w", path, err)
	}
